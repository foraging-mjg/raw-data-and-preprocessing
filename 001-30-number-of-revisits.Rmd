...
e1_nrevisits_ANOVA <- 
  ezANOVA(data=e1_nrevisits_PARTICIPANT_MEANS,
          dv=mean_nrevisits_per_stage,
          wid=pp,
          within=c(resources,stage),
          type=3)

e1_nrevisits_ANOVA_TABLE <-
  e1_nrevisits_ANOVA$ANOVA %>% 
  select(-ges) %>% 
  gt() %>% 
  tab_header(
...
gtsave(e1_nrevisits_ANOVA_TABLE, "e1_tables/e1_nrevisits_ANOVA.png")


# Two points along the x axis, each participant contributes one point per cell
e1_nrevisits_PLOT2 <-
  ggplot(
  data=e1_nrevisits_PARTICIPANT_MEANS, 
...
ggsave("e1_plots/e1_nrevisits_PLOT2.png")
```

```{r out.width="50%"}
knitr::include_graphics("e1_tables/e1_nrevisits_ANOVA.png")
```

```{r out.width="75%"}
knitr::include_graphics("e1_plots/e1_nrevisits_PLOT2.png")
```
